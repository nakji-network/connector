...
func NewConnector() *Connector {
	...
	// Read config from config yaml under `rpcs.solana.full`
	...
	return &Connector{rpcCli, wsCli, conf}
}
...
func (c *Connector) ProgramListener(ctx context.Context, programId solana.PublicKey, namespace string, processTransaction func(*rpc.GetTransactionResult, *solana.Transaction, solana.Signature)) {
	...
	for {
		...
		signatures, _ := c.GetSignaturesForAddressWithOpts(context.Background(), programId, opts)
		...
		for _, txSignature := range signatures {
			...
			txResult, tx := c.GetTransaction(ctx, signature)
...
//func (c *Connector) ProgramBackfiller(ctx context.Context, programId solana.PublicKey, namespace string, processTransaction func(*rpc.GetTransactionResult, *solana.Transaction, solana.Signature), backfillLimit int) {
...
//}
//
//func (c *Connector) backfill(ctx context.Context, programId solana.PublicKey, namespace string, processTransaction func(*rpc.GetTransactionResult, *solana.Transaction, solana.Signature), storeCheckpoint bool, opts *rpc.GetSignaturesForAddressOpts) {
...
//}
//
//func (c *Connector) getSignaturesForAddressOpts(namespace string) (*rpc.GetSignaturesForAddressOpts, bool) {
...
func (c *Connector) GetTransaction(ctx context.Context, signature solana.Signature) (*rpc.GetTransactionResult, *solana.Transaction) {
	...
	err := c.withRetry(
		func() error {
			var err error
			txResult, err = c.rpcClient.GetTransaction(ctx, signature, opts)
			return err
...
func (c *Connector) GetMint(ctx context.Context, mintPubKey solana.PublicKey) *token.Mint {
	var mint token.Mint

	err := c.withRetry(
		func() error {
			err := c.GetAccountDataBorshInto(ctx, mintPubKey, &mint)
			return err
...
func (c *Connector) GetMetadata(ctx context.Context, mintPubKey solana.PublicKey) (*token_metadata.Metadata, error) {
	...
	err = c.withRetry(
		func() error {
			err := c.GetAccountDataBorshInto(ctx, metaPubKey, &meta)
			return err
		}, "GetMeta", map[string]string{"metadata": metaPubKey.String()},
		retry.Delay(c.lowPrioRetryDelay()),
		retry.Attempts(c.lowPrioRetryAttempts()),
	)
...
func (c *Connector) GetNFTTokenAccount(ctx context.Context, tokenMint solana.PublicKey) *token.Account {
	tokenAccountsResult, err := c.GetTokenLargestAccounts(ctx, tokenMint, rpc.CommitmentFinalized)
	if err != nil {
	...
	tokenAccount, err := c.GetTokenAccount(ctx, address)
	if err != nil {
...
func (c *Connector) GetTokenAccount(ctx context.Context, pubKey solana.PublicKey) (*token.Account, error) {
	var tokenAccount token.Account

	err := c.withRetry(
		func() error {
			err := c.GetAccountDataInto(ctx, pubKey, &tokenAccount)
			return err
...
func (c *Connector) GetTokenInstructions(txResult *rpc.GetTransactionResult, tx *solana.Transaction) []*token.Instruction {
...
func (c *Connector) withRetry(retryFunc retry.RetryableFunc, funcName string, logs map[string]string, opts ...retry.Option) error {
	maxRetries := c.Config.GetUint("solana.maxRetries")
	retryDelay := c.Config.GetInt("solana.retryDelay")
...
func (c *Connector) lowPrioRetryDelay() time.Duration {
	maxRetries := c.Config.GetInt("solana.lowPrioRetryDelay")
	return time.Duration(maxRetries) * time.Millisecond
}

func (c *Connector) lowPrioRetryAttempts() uint {
	return c.Config.GetUint("solana.lowPrioMaxRetries")
}
